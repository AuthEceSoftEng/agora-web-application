angular.module('AgoraModule').controller('AgoraCtrlAdvanced', ['fileService', '$scope', '$location', function(files, $scope, $location){
    ...
    if(!$scope.tclass.access) $scope.tclass.access = "default";
    //if(!$scope.tclass.interface) $scope.tclass.interface = false;
    if(!$scope.tclass.methods){
    	$scope.tclass.methods = [{'id':'method1'}];
    	$scope.tclass.methods[0].access = "none";
    	$scope.tclass.methods[0].parameters = [{'id':'parameter1'}];
    }
    if(!$scope.tclass.variables){
    	$scope.tclass.variables = [{'id':'variable1'}];
	...
	$scope.addNewMethod = function() {
		...
		$scope.tclass.methods.push({'id':'method'+newItemNo});
		$scope.tclass.methods[$scope.tclass.methods.length-1].parameters = [{'id':'parameter1'}];
	...
	$scope.addNewParameter = function(method) {
		...
		method.parameters.push({'id':'parameter'+newItemNo});
	};
	...
	$scope.addNewVariable = function() {
		...
		$scope.tclass.variables.push({'id':'variable'+newItemNo});
	...
	$scope.createquery = function(){
		...
		if ($scope.tclass.name)
			shouldquery.push({"match": {"code.class.name": $scope.tclass.name}});
		if ($scope.tclass.interface)
			shouldquery.push({"match": {"code.class.type": "interface"}});
		if ($scope.tclass.extends)
			shouldquery.push({"match": {"code.class.extends": $scope.tclass.extends}});
		if ($scope.tclass.implements)
			shouldquery.push({"match": {"code.class.implements": $scope.tclass.implements}});
		if ($scope.tclass.imports)
			shouldquery.push({"match": {"code.imports": $scope.tclass.imports}});
		if ($scope.tclass.package)
			shouldquery.push({"match": {"code.package": $scope.tclass.package}});
		if ($scope.tclass.access !== "default")
			shouldquery.push({"match": {"code.class.modifiers": $scope.tclass.access}});
		if ($scope.tclass.final)
			shouldquery.push({"match": {"code.class.modifiers": "final"}});
		if ($scope.tclass.abstract)
			shouldquery.push({"match": {"code.class.modifiers": "abstract"}});

		for (var i=0; i<$scope.tclass.variables.length; i++){
			var avariable = $scope.tclass.variables[i];
			shouldquery.push({"nested": {"path": "code.class.variables"}});
			...
			if (avariable.name)
				varshouldquery.push({"match": {"code.class.variables.name": avariable.name}});
			if (avariable.type)
				varshouldquery.push({"match": {"code.class.variables.type": avariable.type}});
			if (avariable.access !== "none")
				varshouldquery.push({"match": {"code.class.variables.modifiers": avariable.access}});
			if (avariable.final)
				varshouldquery.push({"match": {"code.class.variables.modifiers": "final"}});
			if (avariable.abstract)
				varshouldquery.push({"match": {"code.class.variables.modifiers": "abstract"}});
			if (avariable.static)
				varshouldquery.push({"match": {"code.class.variables.modifiers": "static"}});
		...
		for (var i=0; i<$scope.tclass.methods.length; i++){
			...
			shouldquery.push({"nested": {"path": "code.class.methods"}});
			shouldquery[shouldquery.length-1].nested.query = {};
			shouldquery[shouldquery.length-1].nested.query.bool = {};
			shouldquery[shouldquery.length-1].nested.query.bool.should = [];
			metshouldquery = shouldquery[shouldquery.length-1].nested.query.bool.should;
			if (amethod.name)
				metshouldquery.push({"match": {"code.class.methods.name": amethod.name}});
			if (amethod.returntype)
				metshouldquery.push({"match": {"code.class.methods.returntype": amethod.returntype}});
			if (amethod.access !== "none")
				metshouldquery.push({"match": {"code.class.methods.modifiers": amethod.access}});
			if (amethod.final)
				metshouldquery.push({"match": {"code.class.methods.modifiers": "final"}});
			if (amethod.abstract)
				metshouldquery.push({"match": {"code.class.methods.modifiers": "abstract"}});
			if (amethod.static)
				metshouldquery.push({"match": {"code.class.methods.modifiers": "static"}});
			if (amethod.throws)
				metshouldquery.push({"match": {"code.class.methods.throws": amethod.throws}});
			for (var j=0; j<amethod.parameters.length; j++){
				var aparameter = amethod.parameters[j];
				metshouldquery.push({"nested": {"path": "code.class.methods.parameters"}});
				...
				if (aparameter.name)
					parshouldquery.push({"match": {"code.class.methods.parameters.name": aparameter.name}});
				if (aparameter.type)
					parshouldquery.push({"match": {"code.class.methods.parameters.type": aparameter.type}});
			    ...
			    for(var key in tree) {
			        if (key != '_id') {
			            ...
			            if (recurseTree(child, thekeys, thevalues)){
			            	if (Object.keys(tree)[0] != "path"/* && Object.keys(tree)[0] != "code.class.methods.returntype"*/){//do not highlight returntype
	...
	$scope.search = function(){
		...
		if ($scope.tclass){
			...
			$location.search({'q': JSON.stringify($scope.tclass)});
			$scope.loadMore();
		}
		else{
	...
	function highlightString(thestring, theterm){
		return thestring.replaceAll(theterm, 
				"<eshi>" + theterm + "</eshi>").replaceAll(theterm.toUpperCase(), 
						"<eshi>" + theterm.toUpperCase() + "</eshi>").replaceAll(theterm.capitalize(), 
						"<eshi>" + theterm.capitalize() + "</eshi>");
	}
	...
	$scope.loadMore = function(){
		...
		function strung(thekey, iters, arg) {
			...
			if (thekey == "class"){
	            ...
	            ret += iters + classmodifiers + classtype + " " + classname + 
	            strung("extends", iters + "\t", arg.extends) + 
	            strung("implements", iters + "\t", arg.implements) + "{";
		        ...
		        arg.forEach(function(elem, index) {
		            ...
		            ret += '\n' + iters + strung("modifiers", iters + "\t", elem.modifiers) + methodtype + methodname + 
		            	"(" + strung("parameters", iters + "\t", elem.parameters) + ")" 
		            	+ (elem.throws && elem.throws.length > 0 ? strung("throws", iters + "\t", elem.throws) : "")+ ";";
		        });
			}
