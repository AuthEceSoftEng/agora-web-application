angular.module('AgoraModule').directive('myView', function ($compile) {
	return {
		...
		link:function (scope, elm, attrs) {
			elm.append($compile(
					'<a class="btn" href=\"/file#?file=' + attrs.datapath + '\"' +
					'target=\"_blank\">' + attrs.dataname + '</a>'
			)(scope));
		}
	};
});
...
angular.module('AgoraModule').directive('myProjectView', function ($compile) {
	return {
		...
		link:function (scope, elm, attrs) {
			elm.append($compile(
					'<a class="btn" href=\"/project#?project=' + attrs.datapath + '\"' +
					'target=\"_blank\">' + attrs.dataname + '</a>'
			)(scope));
		}
	};
		...
		link:function (scope, elm, attrs) {
			...
			function getUrl(){
				if (isIE() && isIE() <= 9)
					...
					saveAs(new Blob([attrs.datacontent], {type: "text/java"}), attrs.dataname);
				return false;
			...
			elm.append($compile(
					'<a class="btn" download="' + attrs.dataname + '">' + 'Download file' + '</a>'
